...
function GetTempVerzeichnis: string;
...
function GetTempVerzeichnis: string;
var
  p: array[0..256 - 1] of Char;
  iLen, i: Integer;
begin
  Result := '';
  iLen := GetTempPath(255, p);
  if iLen > 0 then
  begin
    StrLCopy(p, p, iLen);
...
begin
  ...
  try
    ...
    begin
      ATempValue := AnsiReplaceStr(Value,'%temp%',GetTempVerzeichnis); // changed by dkraaz
      SetLogFile(ATempValue);    // changed by adasen
    end;
  finally
    LeaveCriticalSection(FCriticalAppender);
  end;
...
begin
  ...
  try
    try
      ATempValue := AnsiReplaceStr(FileName,'%temp%',GetTempVerzeichnis); // changed by dkraaz
      //showMessage('FileName: ' + ATempValue);
      Props.LoadFromFile(ATempValue);
